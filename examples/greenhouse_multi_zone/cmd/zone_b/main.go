// Zone B sensors. Publishes: zone_b_reading. Subscribes: zone_b_command.
package main
...
func main() {
	handle, z, err := framework.Boot(func(bus *framework.Bus) *ZoneSensors {
		return &ZoneSensors{bus: bus, temp: 25, humidity: 65, soil: 45, zone: "zone_b"}
	})
	if err != nil {
		panic(err)
	}
	...
	for {
		...
		handle.Broadcast("zone_b_reading", reading)
